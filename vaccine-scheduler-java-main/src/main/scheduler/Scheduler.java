...
public class Scheduler {
    ...
    private static boolean usernameExistsPatient(String username) {
        ...
        try {
            ...
            return resultSet.isBeforeFirst(); // what does this mean
        } catch (SQLException e) {
    ...
    private static void reserve(String[] tokens) {
        ...
        // seeing if the format of the input is correct

        //Seeing if there are available vaccines --> if not, output the error message
        ConnectionManager cm = new ConnectionManager();
        Connection con = cm.createConnection();
        try {
            ...
            // if there are not enough available doses
            if(vaccineDoses == 0){
        ...
        // checking the caregivers that are available on that date
        try {
            ...
            statementD.setString(2,CaregiverName);
            statementD.executeUpdate();

            // print out
            String retrieveAppointmentID = "SELECT AptId FROM Appointments WHERE AptTime = ? AND CaregiverName = ? ";
            PreparedStatement statementR = con.prepareStatement(retrieveAppointmentID);
            statementR.setDate(1, d);
            statementR.setString(2,CaregiverName);
            ResultSet resultSet2 = statementR.executeQuery();
            while(resultSet2.next()) {
                int AptId = resultSet2.getInt("AptId");
                System.out.println("Appointment ID: " + AptId + ", Caregiver username: " + CaregiverName);
            }
    ...
    private static void showAppointments(String[] tokens){
        ...
        if(currentCaregiver != null){
            ConnectionManager cm = new ConnectionManager();
            Connection con = cm.createConnection();
            String getApts = "SELECT AptId, VaccineName, AptTime, PatientName FROM Appointments WHERE CaregiverName = ? ORDER BY AptId";
            try{
                PreparedStatement statement = con.prepareStatement(getApts);
                ...
                while(resultSet.next()){
                    ...
                    Date AptTime = resultSet.getDate("AptTime");
                    String PatientName = resultSet.getString("PatientName");

                    System.out.println(AptId + " " + VaccineName + " " + AptTime + " " + PatientName);
                }
            ...
            ConnectionManager cm = new ConnectionManager();
            Connection con = cm.createConnection();
            String getApts = "SELECT AptId, VaccineName, AptTime, CaregiverName FROM Appointments WHERE PatientName = ? ORDER BY AptId";
            try {
                PreparedStatement statement = con.prepareStatement(getApts);
                ...
                while(resultSet.next()){
                    ...
                    System.out.println(AptId + " " + VaccineName + " " + AptTime + " " + CaregiverName);
                }
